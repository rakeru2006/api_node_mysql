...
var connection = mysql.createConnection({
 ...
 figlet(' Employee App', (err, result) => {
...
addEmployee = () => {
 ...
 inquirer.prompt([
   ...
   },
   {
     ...
     choices: function() {
       var choiceArray = [];
       for (var i = 0; i < roleOpt.length; i++) {
         choiceArray.push(roleOpt[i].title)
       }
       return choiceArray;
     }
   },
   ...
   for (i = 0; i < roleOpt.length; i++) {
     if (roleOpt[i].title === answer.role_id) {
       role_id = roleOpt[i].id
     }
   }
 ...
 inquirer.prompt([
   {
     ...
     choices: ["Departaments", "Roles", "Employees", "Exit"]
   }
 ]).then(answer => {
   if (answer.viewChoice === "Departaments") {
     viewDepartments();
   }
...
updateSomething = () => {
 inquirer.prompt([
   ...
   if (answer.update === "Update employee roles") {
     ...
     figlet('Thanks for use  Employee App', (err, result) => {
       console.log(err || result);
     });
...
for (i = 0; i < roleOpt.length; i++) {
 if (answer.newRole === roleOpt[i].title) {
...
deleteRole = () => {
 ...
 inquirer.prompt([
   ...
   for (i = 0; i < roleOpt.length; i++) {
     if (answer.deleteRole === roleOpt[i].title) {
       newChoice = roleOpt[i].id
